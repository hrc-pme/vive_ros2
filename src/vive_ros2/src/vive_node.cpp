...
// Initialize socket address structure for TCP connection
    ...
    std::map<std::string, rclcpp::Publisher<geometry_msgs::msg::TransformStamped>::SharedPtr> dynamic_abs_publishers_;
    std::map<std::string, rclcpp::Publisher<geometry_msgs::msg::TransformStamped>::SharedPtr> dynamic_rel_publishers_;
    ...
    void createPublishers() {
        ...
        RCLCPP_INFO(this->get_logger(), "Dynamic 8-Tracker infrastructure initialized.");
    }
    ...
    void establishServerConnection() {
        ...
        if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
            RCLCPP_WARN(this->get_logger(), "連線失敗！Vive Server 可能尚未啟動。將由斷線重連機制接管...");
            close(sock);
            sock = -1;
        } else {
            RCLCPP_INFO(this->get_logger(), "成功連線至 Vive Server！");
        }
    }
    ...
    void reconnect() {
        ...
        while (sock < 0 && rclcpp::ok()) {
            RCLCPP_WARN(this->get_logger(), "連線中斷或尚未開啟，1 秒後嘗試重新連線...");
    ...
    std::string generateFrameId(ControllerRole role, bool isRelative) {
        std::string prefix = "tracker" + std::to_string((int)role + 1);
    ...
    void publishTransform(const VRControllerData& pose, bool isRelative = false) {
        ...
        std::string frame_prefix = "tracker" + std::to_string((int)role + 1);
        
       
        transformVRToROS(pose, transformStamped);

       
        transformStamped.child_frame_id = frame_prefix + "_frame";
        ...
        if (isRelative) {
            if (dynamic_rel_publishers_.find(frame_prefix) == dynamic_rel_publishers_.end()) {
                std::string topic = frame_prefix + "/vive_pose_rel";
                dynamic_rel_publishers_[frame_prefix] = this->create_publisher<geometry_msgs::msg::TransformStamped>(topic, VRConfig::DEFAULT_QUEUE_SIZE);
                RCLCPP_INFO(this->get_logger(), "Dynamically created publisher for: %s", topic.c_str());
            }
            dynamic_rel_publishers_[frame_prefix]->publish(roleSpecificMsg);
        } else {
            if (dynamic_abs_publishers_.find(frame_prefix) == dynamic_abs_publishers_.end()) {
                std::string topic = frame_prefix + "/vive_pose_abs";
                dynamic_abs_publishers_[frame_prefix] = this->create_publisher<geometry_msgs::msg::TransformStamped>(topic, VRConfig::DEFAULT_QUEUE_SIZE);
                RCLCPP_INFO(this->get_logger(), "Dynamically created publisher for: %s", topic.c_str());
            }
            dynamic_abs_publishers_[frame_prefix]->publish(roleSpecificMsg);
        }
    ...
    VRControllerData filterPose(const VRControllerData& pose) {
        ...
        static VRControllerData prevPoses[8];
        static bool initialized[8] = {false, false, false, false, false, false, false, false};
        
        int role_idx = (int)pose.role;
        if (role_idx < 0 || role_idx >= 8) return pose;
        
        if (!initialized[role_idx]) {
            prevPoses[role_idx] = pose;
            initialized[role_idx] = true;
            return pose;
        }

        VRControllerData filteredPose = VRUtils::filterPose(pose, prevPoses[role_idx], VRConfig::POSE_SMOOTHING_FACTOR);
        prevPoses[role_idx] = filteredPose;
...
public:
	...
	if (role == 0) prefix = "tracker1";
        else if (role == 1) prefix = "tracker2";
        else if (role == 2) prefix = "tracker3";
        else if (role == 3) prefix = "tracker4";
        else if (role == 4) prefix = "tracker5";
        else if (role == 5) prefix = "tracker6";
        else if (role == 6) prefix = "tracker7";
        else if (role == 7) prefix = "tracker8";
        else prefix = "unknown_tracker";
       ...
       char buffer[VRConfig::BUFFER_SIZE] = {0};
        while (rclcpp::ok()) {
            ...
            if (receivedByteCount > 0) {
                ...
                RCLCPP_WARN(this->get_logger(), "與 Vive Server 斷開連線！啟動自動重連機制...");
                reconnect();
                
            } else {
                RCLCPP_ERROR(this->get_logger(), "網路讀取失敗 (Read error)！嘗試強制重新連線...");
                reconnect();
                continue; // 
            }
